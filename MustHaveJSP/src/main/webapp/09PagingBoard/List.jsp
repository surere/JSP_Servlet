...
/*
	검색폼에서 입력한 (목록보기 form method 부분) 검색어와 필드명을 파라미터로 받아온다.
...
// 	int totalCount = dao.selectCount(param); //게시물 수 확인
	...
	*/
	...
	/*
		...
		List.jsp?pageNum= => 이와같이 파라미터는 있는데 값이 없을때는 
			빈값으로 체크된다. 따라서 아래 if문은 두개의 조건으로 구성해야 한다.
	*/
	//(현재페이지)
	int pageNum = 1;
...
<%--     <h2>목록 보기(List) - 현재 페이지 : <%= pageNum %>(전체 : <%= totalPage %>)</h2> --%>
    <!-- 현재 페이지 : <%= pageNum %>(전체 : <%= totalPage %> 부분을 따로 넣는다. -->
    <!-- 검색 폼 -->
    <form method="get">  
...
<%
	...
	// 출력할 게시물이 있는 경우에는 확장for문으로 List컬렉션에
	// 저장된 데이터의 갯수만큼 반복하여 출력한다.
    int virtualNum = 0; 
	
	// 페이지가 적용된 가상 번호를 계산하기 위해 생성한 변수 (LowNum처럼?)
	int countNum = 0;
...
//         virtualNum = totalCount--;   전체 게시물 수에서 시작해 1씩 감소
		...
		// 전체 게시물수 - (((현재페이지-1)*한페이지출력갯수) + countNum증가치)
        ...
        <tr align="right">
        	...
        	<!-- 
        		...
        		request.getRequestURI() : request내장객체를 통해 현재페이지의
        			...
        			경로명을 통해 "경로명.jsp?pageNum=페이지번호와 같은
        			바로가기 링크를 생성한다.
        	 -->
        <%
