...
<!-- 	인수형 문자형으로 set해서 변수 선언을 함 -->
	...
	따라서 Java의 else문과 같은 구문이 된다.  --><!-- 자바에서 else문에 해당 -->
	...
	<c:if test="${ true }" var= "result5">
		EL 양쪽에 빈 공백이 있는 경우 false<br/><!-- test= "${ true }"이면 
		true. 출력된다.>
	</c:if>
	result5 : ${ result5 }<br/>
	...
	<from method="get">
		아이디 : <input type= "txet" name="user"/>
		<br/>
		패스워드 : <input type= "txet" name="pass"/>
		<br/>
		...
		<c:if test="${param.user=='seoulit' and param.pass eq '1234' }"
		var="loginResult">${ param.user }님, 반갑습니다!.<br/>
		</c:if>
